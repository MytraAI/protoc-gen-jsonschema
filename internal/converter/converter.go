...
func (c *Converter) convertEnumType(enum *descriptor.EnumDescriptorProto, converterFlags ConverterFlags) (jsonschema.Schema, error) {
	...
	if src := c.sourceInfo.GetEnum(enum); src != nil {
		jsonSchemaType.Title, jsonSchemaType.Description, _ = c.formatTitleAndDescription(strPtr(enum.GetName()), src)
	}
	...
	for _, value := range enum.Value {
		...
		if src := c.sourceInfo.GetEnumValue(value); src != nil {
			_, valueDescription, _ = c.formatTitleAndDescription(nil, src)
		}
